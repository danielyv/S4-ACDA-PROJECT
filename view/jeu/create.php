<form method="POST" action="index.php" id="form_id" enctype="multipart/form-data" class="col-md-10">
  <fieldset>
    <legend>Ajouter un jeu :</legend>
    <div class="form-group">
    <input type='hidden' name='action' value='created'>
    <input type='hidden' name='controller' value='jeu'>
    <div class="form-group">
      <label class="col-lg-2 control-label" for="nomJeu_id">Nom du jeu</label>
      <input class="form-control connect" id="nomJeu_id" type="text" name="nomJeu" placeholder="Ex : Firewatch" required>
    </div>
    <div class="form-group">
      <label class="col-lg-2 control-label" for="categorie_id">Catégorie</label>
      <input class="form-control connect" id="categorie_id" type="text" name="categorie" placeholder="Ex : RPG" required>
    </div>
    <div class="form-group">
      <label class="col-lg-2 control-label">Jeu validé </label>
      <input type="checkbox" class="form-check-input" name="valide" value="1">
    </div>
    <div class="form-group">
      <label for="image_id" class="col-lg-2 control-label">Image
        <img class="detailUpload" data-toggle="tooltip" data-placement="top" title="Max size : 720px*720px. Formats supprotés : .jpg .jpeg .gif .png"  src="data:image/svg+xml;utf8;base64," />
      </label>
      <div class="col-lg-10">
        <input type="file" accept="image/*" class="form-control" id="image_id" name="image" placeholder="Importer une image">
      </div>
    </div>
    <div class="form-group">
     <label for="descriptif_id" class="col-lg-2 control-label">Description</label>
     <textarea class="form-control" id="descriptif_id" name="descriptif"></textarea>
     <script>
        CKEDITOR.replace( 'descriptif_id' );
     </script>
   </div>
  </fieldset>
      <button type="reset" class="btn btn-default">Annuler</button>
      <button type="submit" class="btn btn-primary">Accepter</button>
</form>
